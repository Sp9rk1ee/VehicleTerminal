    ...
    QWidget(parent),cursorGlobalPos(0,0),isMousePress(false)
{
    ...
    this->setWindowModality(Qt::ApplicationModal);//应用模态,即在软键盘弹出时，用户必须与软键盘交互后才能返回到主应用界面。
    //下面就是调用一系列自定义的函数
    //初始化ui显示
    this->initStyleSheet();
    ...
    globalVLayout->setMargin(2);// 设置布局的外边距为2
    globalVLayout->setSpacing(0);// 设置布局的间距为0
    globalVLayout->addWidget(inputBufferArea,1);//第二个参数决定了每个部件在布局中所占用的比例
    globalVLayout->addWidget(functionAndCandidateArea,1);
    globalVLayout->addWidget(keysArea,5);

    readDictionary();//读拼音字典
    showInputBufferArea();//显示输入缓冲区
}
...
{
    inputBufferArea->setVisible(true);//输入缓冲区可见，这个其实是上面那一整条，就是包含了编辑框和pleaseinput的那一整条
    inputTitleLabel->setText(inputTitle);//输入框前面的那个标签设置为inputTitle
    inputContentEdit->setText(inputContent);//输入编辑框的内容设置为inputContent，这个才是真正的pleaseinput旁边的那个框框
    inputContentEdit->setFocus();//setFocus() 是将焦点设置到 inputContentEdit 编辑框。这意味着在调用该函数后，用户可以立即开始在该文本框中输入内容
...
void SoftKeyboard::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(event->buttons()&Qt::LeftButton && isMousePress && isMoveEnabled)
    {
        ...
        /*
        this->pos(): 获取当前窗口的当前位置（相对于其父窗口或者屏幕的坐标）。
        event->globalPos(): 获取当前鼠标的位置，返回的是全局坐标（相对于整个屏幕的坐标）。
        cursorGlobalPos: 这是一个保存了鼠标按下时的全局坐标的位置变量，它表示鼠标按下时的初始位置。
        */
        QPoint position = this->pos() + event->globalPos() - cursorGlobalPos;
        /*
        在鼠标按下并拖动时，计算鼠标相对于窗口的移动偏移。
        根据这个偏移量实时更新窗口的位置，使得窗口能够随着鼠标的移动而拖动。
        更新鼠标按下时的初始位置 cursorGlobalPos，为下一次计算准备
        */
...
void SoftKeyboard::initNumberLetterBtn()
{
    ...
    for(int i=0;i<36;i++)//为10个数字，26个字母按键申请空间,连接信号与槽
    {
        ...
        numberLetterBtn[i]->setToolButtonStyle(Qt::ToolButtonTextOnly);//设置按钮的显示样式为仅显示文本。这样，按钮上将只显示文字,没有图标
        numberLetterBtn[i]->setSizePolicy(QSizePolicy::Preferred,QSizePolicy::Preferred);//设置按钮的尺寸策略为Preferred,这意味着按钮的大小由其内容决定
        connect(numberLetterBtn[i],SIGNAL(clicked()),this,SLOT(numberLetterBtnSlot()));//按钮被点击时触发槽函数
    }
}
/*
...
void SoftKeyboard::initSpecialBtn()
{
    ...
    connect(upperOrLowerBtn,SIGNAL(clicked()),this,SLOT(changeUpperLowerSlot()));//切换大小写，点击时触发槽函数
    ...
    deleteBtn->setAutoRepeatDelay(300);//当用户按下 deleteBtn 按钮时，按钮不会立即重复触发事件，而是会等待 300 毫秒 后
    deleteBtn->setAutoRepeatInterval(60);//设置自动重复触发事件的时间间隔。也就是说，一旦自动重复开始触发，按钮将每 60 毫秒 触发一次点击事件。
    deleteBtn->setAutoRepeat(true);//启用自动重复功能，按钮会在被按下并保持按住时自动重复触发 clicked() 信号
    connect(deleteBtn,SIGNAL(clicked(bool)),this,SLOT(deleteTextSlot()));

    skinBtn = new QToolButton();//切换皮肤
    ...
    letterOrSymbolBtn = new QToolButton();//切换字母和符号
    ...
    chOrEnBtn = new QToolButton();//中英文切换
...
void SoftKeyboard::initFunctionAndCandidateArea()
{
    ...
    candidateLetter->setFrame(false);//去掉外面的边框
    candidateLetter->setEnabled(false);//设为不可用。这样用户就不能在这个控件中输入任何文本，通常用于只读的文本框或显示信息的区域
    candidateLetter->setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);//水平方向固定大小
    connect(candidateLetter,SIGNAL(textChanged(QString)),this,SLOT(candidateLetterChangedSlot(QString)));//文本内容改变时触发
    //候选词、翻页按钮显示区域
...
void SoftKeyboard::readDictionary()
{
    ...
    else
    {
        ...
        {   //假如一行是最初zui'chu，那么pinyinPosition指向z所在的位置，linPinyin=zui'chu，lineChinese=最初
            lineText = QString(QString::fromUtf8(pinyinFile.readLine()));
            pinyinPosition=regExp.indexIn(lineText,0);//使用正则表达式 regExp 查找 lineText 中第一个匹配的位置。
